...
@interface singlePresentViewController ()

@end

@implementation singlePresentViewController
...
- (void)viewDidLoad {
    ...
    if (self.can_redeem == NO) {
        self.onlineRedeemButton.enabled = NO;
    }
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onOnlineRedeemNotification:) name:OnlineRedeemNotification object:nil];
}
...
- (void)onOnlineRedeemNotification:(NSNotification*)notify
{
    if ([[notify.object objectForKey:@"ERR_CODE"] integerValue] == 35)
    {
        SCLAlertView *alert = [[SCLAlertView alloc]initWithNewWindow];
        [alert showSuccess:NSLocalizedString(@"立即兌換獎品成功", nil)  subTitle:NSLocalizedString(@"將會直接寄送給您", nil) closeButtonTitle:NSLocalizedString(@"確定", nil) duration:0.0f];
...
- (void)showPopUpView
{
    ...
    self.nameField.layer.masksToBounds = NO;
    self.phoneNumberField.layer.cornerRadius = 10;
    self.phoneNumberField.layer.masksToBounds = NO;
    self.addressField.layer.cornerRadius = 10;
    self.addressField.layer.masksToBounds = NO;
}
- (IBAction)obtainPressed:(id)sender {
    
    if (self.nameField.text.length > 0 && self.phoneNumberField.text.length >0  && self.addressField.text.length > 0)
    {
        [[VintageApiService sharedInstance]onlineRedeem:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] my_gift_id:[self.selectedEventArray valueForKey:@"my_gift_id"] name:self.nameField.text phone:self.phoneNumberField.text address:self.addressField.text];
        [self.view endEditing:YES];
    }
    else
    {
        SCLAlertView *alert = [[SCLAlertView alloc]initWithNewWindow];
        [alert showWarning:NSLocalizedString(@"請輸入完整收件資訊", nil)  subTitle:nil closeButtonTitle:NSLocalizedString(@"確定", nil) duration:0.0f];
        [self.view endEditing:YES];
    }
        ...
        [UIView animateWithDuration:0.25
                         ...
                         animations:(void (^)(void)) ^{
                             self.view.frame = CGRectMake(self.view.frame.origin.x,
                                                          screenHeight - kbSize.height - self.view.frame.size.height + self.nameField.frame.size.height+20,
