...
@interface WineOverViewController ()
{
    ...
    UIView *maskView;
    UIView* nilResultView;
    UILabel* nilResultLabel;
    DGActivityIndicatorView* activityIndicatorView;
}

@end

@implementation WineOverViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.popUpView.hidden = YES;
    UITapGestureRecognizer* filterRecog = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(showFilterList:)];
    [self.filterButtonView addGestureRecognizer:filterRecog];
    ...
    NSString* limit = @"30";
    self.pageTitle.text = self.pageName;
    if ([self.pageIndex isEqualToString:@"5"]) {
        NSLog(@"fetchOverViewList without searchString and filter on");
    ...
    else if ([self.pageIndex isEqualToString:@"0"])
    {
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onFetchingWineListInfoNotification:) name:FetchingNewItemListNotification object:nil];
        [self fetchNewItemList:limit offset:initialOffset];
        self.searchButton.hidden = YES;
    }
    else if ([self.pageIndex isEqualToString:@"1"])
    {
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onFetchingWineListInfoNotification:) name:FetchingExpertListNotification object:nil];
        [self fetchExpertList:limit offset:initialOffset];
        self.searchButton.hidden = YES;
    }
    else if ([self.pageIndex isEqualToString:@"2"])
    {
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onFetchingWineListInfoNotification:) name:FetchingHotItemListNotification object:nil];
        [self fetchHotItemList:limit offset:initialOffset];
        self.searchButton.hidden = YES;
    }
    else if ([self.pageIndex isEqualToString:@"3"])
    {
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onFetchingWineListInfoNotification:) name:FetchingSeasonItemListNotification object:nil];
        [self fetchSeasonList:limit offset:initialOffset];
        self.searchButton.hidden = YES;
    }
    else if ([self.pageIndex isEqualToString:@"4"])
    {
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onFetchingWineListInfoNotification:) name:FetchingGiftItemListNotification object:nil];
        [self fetchGiftList:limit offset:initialOffset];
        self.searchButton.hidden = YES;
    }
    else if ([self.pageIndex isEqualToString:@"6"])
    {
        ...
        [self fetchWaiterResultList:limit offset:initialOffset option_ids:self.waiterDecisionString];
        self.searchButton.hidden = YES;
    }
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter]removeObserver:@"FetchingNewItemListNotification"];
    [[NSNotificationCenter defaultCenter]removeObserver:@"FetchingExpertListNotification"];
    [[NSNotificationCenter defaultCenter]removeObserver:@"FetchingHotItemListNotification"];
    [[NSNotificationCenter defaultCenter]removeObserver:@"FetchingSeasonItemListNotification"];
    [[NSNotificationCenter defaultCenter]removeObserver:@"FetchingGiftItemListNotification"];
}
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([self.pageIndex isEqualToString:@"6"])
    {
        self.searchButton.hidden = YES;
    }
    else
    ...
    if ([[defaults objectForKey:@"filterSwitch"]isEqualToString:@"on"]||[[defaults objectForKey:@"searchSwitch"]isEqualToString:@"on"])
    {
        ...
        if ([[defaults objectForKey:@"searchSwitch"]isEqualToString:@"on"]) {
            if ([self.pageIndex isEqualToString:@"5"]) {
                NSLog(@"fetchOverViewList with searchString and filter on");
...
- (void)showFilterList:(UITapGestureRecognizer*)recog
{
    [self performSegueWithIdentifier:@"overToFilter" sender:self];
}

- (void)insertRowAtBottom
{
    __weak WineOverViewController *weakSelf = self;
...
- (void)onFetchingWineListInfoNotification:(NSNotification*)notify
{
    ...
    activityIndicatorView = [[DGActivityIndicatorView alloc]initWithType:DGActivityIndicatorAnimationTypeDoubleBounce tintColor:[UIColor grayColor] size:40.0f];
    activityIndicatorView.frame = CGRectMake(0, 0, 50.0f, 50.0f);
    activityIndicatorView.center = self.view.center;
    [self.view addSubview:activityIndicatorView];
    [activityIndicatorView startAnimating];
    // setup infinite scrolling
    [nilResultView removeFromSuperview];
    if ([[defaults objectForKey:@"filterSwitch"]isEqualToString:@"on"]||[[defaults objectForKey:@"searchSwitch"]isEqualToString:@"on"])
    {
        if ([[defaults objectForKey:@"filtFirstTime"]isEqualToString:@"on"]||[[defaults objectForKey:@"searchFirstTime"]isEqualToString:@"on"])
        {
            if (item.count == 0)
            {
                nilResultView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 150, 30)];
                nilResultLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 150, 20)];
                nilResultLabel.text = @"沒有符合條件的酒";
                nilResultLabel.textAlignment = NSTextAlignmentCenter;
                nilResultLabel.textColor = [UIColor blackColor];
                nilResultLabel.font = [UIFont systemFontOfSize:15];
                nilResultLabel.center = nilResultView.center;
                nilResultView.center = self.view.center;
                [nilResultView addSubview:nilResultLabel];
                [self.view addSubview:nilResultView];
                NSLog(@"no match");
            }
            else
            {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else
    {
        ...
        [cell.itemImageView sd_setImageWithURL:[[wineInfoArray objectAtIndex:indexPath.row] valueForKey:@"pic_url"] placeholderImage:nil options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize)
         {
             [cell.itemImageView addSubview:activityIndicator];
             activityIndicator.center = cell.itemImageView.center;
             [activityIndicator startAnimating];
         } completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL)
         {
             
             /*
             CGRect rect = CGRectMake(0,0,cell.itemImageView.frame.size.width ,cell.itemImageView.frame.size.height);
        ...
        UITextView *serialNum = [[UITextView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(subTitle.frame) -10 , superViewWidth*0.53125, [[title_en_array objectAtIndex:indexPath.row]floatValue])];
        serialNum.text = [[wineInfoArray objectAtIndex:indexPath.row] valueForKey:@"serial_num"];

        serialNum.font = [UIFont systemFontOfSize:12];
        serialNum.textColor = [UIColor colorWithHexString:@"c12558"];
        serialNum.userInteractionEnabled = NO;
        serialNum.contentInset = UIEdgeInsetsMake(0,0,0,0);
        [serialNum sizeToFit];
        
        UIView * priceWrapperView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(serialNum.frame) , [[priceArray objectAtIndex:indexPath.row]floatValue], self.view.frame.size.height * 0.04)];
        ...
        [textWrapperView addSubview:serialNum];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (![[[wineInfoArray objectAtIndex:indexPath.row]valueForKey:@"id" ]isEqualToString:@"0"])
    {
        [self performSegueWithIdentifier:@"overToSingle" sender:self];
...
-(void)tableView:(UITableView*) tableView willDisplayCell:(UITableViewCell*)cell forRowAtIndexPath:(NSIndexPath*)indexPath
{
    if (indexPath.row == [wineInfoArray count] - 1)
    {
        ...
        if ([[defaults objectForKey:@"searchSwitch"]isEqualToString:@"on"]) {
            ...
            {
                [self fetchNewItemList:limit offset:calculatedOffset];
            }
            else if ([self.pageIndex isEqualToString:@"1"])
            {
                [self fetchExpertList:limit offset:calculatedOffset];
            }
            else if ([self.pageIndex isEqualToString:@"2"])
            {
                [self fetchHotItemList:limit offset:calculatedOffset];
            }
            else if ([self.pageIndex isEqualToString:@"3"])
            {
                [self fetchSeasonList:limit offset:calculatedOffset];
            }
            else if ([self.pageIndex isEqualToString:@"4"])
            {
                [self fetchGiftList:limit offset:calculatedOffset];
            }
            else
            {
                [self fetchWaiterResultList:limit offset:calculatedOffset option_ids:self.waiterDecisionString];
            }
        }
        else
        {
...
{
    if ([segue.identifier isEqualToString:@"overToSingle"])
    {
        NSString * language = [[NSLocale preferredLanguages] objectAtIndex:0];
...
- (IBAction)searchAction:(id)sender {
    [self performSegueWithIdentifier:@"overToSearch" sender:self];
}
...
- (void)fetchGiftList:(NSString*)limit offset:(NSString*)offset
{
    if ([[defaults objectForKey:@"filtWithAllOptions"]isEqualToString:@"selected"])
    {
        [[VintageApiService sharedInstance]fetchGiftItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithAllOptions");
    }
    else if ([defaults objectForKey:@"filtWithWineType"]!=nil)
    {
        
        [[VintageApiService sharedInstance]fetchGiftItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:[defaults objectForKey:@"filtWithRegion"] wine_type_id:[defaults objectForKey:@"filtWithWineType"]];
        NSLog(@"filtWithWineType");
        
    }
    else
    {
        [[VintageApiService sharedInstance]fetchGiftItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:@"" order:@"" limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithoutOptions");
    }
}
- (void)fetchSeasonList:(NSString*)limit offset:(NSString*)offset
{
    if ([[defaults objectForKey:@"filtWithAllOptions"]isEqualToString:@"selected"])
    {
        [[VintageApiService sharedInstance]fetchSeasonItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithAllOptions");
    }
    else if ([defaults objectForKey:@"filtWithWineType"]!=nil)
    {
    ...
    {
        [[VintageApiService sharedInstance]fetchSeasonItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:@"" order:@"" limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithoutOptions");
    }
    
}
- (void)fetchHotItemList:(NSString*)limit offset:(NSString*)offset
{
    
    if ([[defaults objectForKey:@"filtWithAllOptions"]isEqualToString:@"selected"])
    {
        [[VintageApiService sharedInstance]fetchHotItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithAllOptions");
    }
    else if ([defaults objectForKey:@"filtWithWineType"]!=nil)
    {
    ...
    {
        [[VintageApiService sharedInstance]fetchHotItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:@"" order:@"" limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithoutOptions");
    }
}
- (void)fetchExpertList:(NSString*)limit offset:(NSString*)offset
{
    
    if ([[defaults objectForKey:@"filtWithAllOptions"]isEqualToString:@"selected"])
    {
        [[VintageApiService sharedInstance]fetchExpertListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithAllOptions");
    }
    else if ([defaults objectForKey:@"filtWithWineType"]!=nil)
    {
    ...
    {
        [[VintageApiService sharedInstance]fetchExpertListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:@"" order:@"" limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithoutOptions");
    }
}
- (void)fetchNewItemList:(NSString*)limit offset:(NSString*)offset
{
    if ([[defaults objectForKey:@"filtWithAllOptions"]isEqualToString:@"selected"])
    {
        [[VintageApiService sharedInstance]fetchNewItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithAllOptions");
    }
    else if ([defaults objectForKey:@"filtWithWineType"]!=nil)
    {
    ...
    {
        [[VintageApiService sharedInstance]fetchNewItemListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:@"" order:@"" limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
        NSLog(@"filtWithoutOptions");
    }
}
- (void)fetchWaiterResultList:(NSString*)limit offset:(NSString*)offset option_ids:(NSString*)option_ids
{
    if ([[defaults objectForKey:@"filtWithAllOptions"]isEqualToString:@"selected"])
    {
        [[VintageApiService sharedInstance]fetchWaiterResultListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:@"" wine_type_id:@"" option_ids:option_ids];
        NSLog(@"filtWithAllOptions");
    }
    else if ([defaults objectForKey:@"filtWithWineType"]!=nil)
    {
        
        [[VintageApiService sharedInstance]fetchWaiterResultListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:[defaults objectForKey:@"filtWithRegion"] wine_type_id:[defaults objectForKey:@"filtWithWineType"]option_ids:option_ids];
        NSLog(@"filtWithWineType");
        
    }
    else
    {
        [[VintageApiService sharedInstance]fetchWaiterResultListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:@"" order:@"" limit:limit offset:offset country_id:@"" wine_type_id:@"" option_ids:option_ids];
        NSLog(@"filtWithoutOptions");
    }
}
