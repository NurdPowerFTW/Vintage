...
- (void)viewDidLoad {
    ...
    [[VintageApiService sharedInstance]setLastTappedIndex:@"4"];
    ...
    self.selectedButtonIdxArray = [[NSMutableArray alloc] init];
    NSString *initialOffset = [NSString stringWithFormat:@"%d",0];
    NSString* limit = @"30";
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onFetchingCollectionListNotification:) name:FetchingCollectionListNotification object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onCancelCollectionListNotification:) name:CancelCollectionNotification object:nil];
    [self fetchCollectionListInfo:limit offset:initialOffset];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter]removeObserver:@"FetchingCollectionListNotification"];
    [[NSNotificationCenter defaultCenter]removeObserver:@"CancelCollectionNotification"];
}
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([[defaults objectForKey:@"filterSwitch"]isEqualToString:@"on"])
    {
...
- (void)onFetchingCollectionListNotification:(NSNotification*)notify
{
    NSArray *item = [notify.object objectForKey:@"wines"];
    
    if ([[defaults objectForKey:@"cancelSwitch"]isEqualToString:@"on"])
    {
        NSLog(@"ready to refresh table after deletion");
        wineInfoArray = [[NSMutableArray alloc]initWithArray:item];
        [self loadDataFromVintageApi:0];
        [self.tableView reloadData];
        [[NSUserDefaults standardUserDefaults]setObject:@"off" forKey:@"cancelSwitch"];
        [[NSUserDefaults standardUserDefaults]synchronize];
    }
    else if ([[defaults objectForKey:@"filterSwitch"]isEqualToString:@"on"])
    {
        if ([[defaults objectForKey:@"filtFirstTime"]isEqualToString:@"on"])
        {
            wineInfoArray = [[NSMutableArray alloc]initWithArray:item];
            [self loadDataFromVintageApi:0];
...
- (void)onCancelCollectionListNotification:(NSNotification*)notify
{
    NSString *initialOffset = [NSString stringWithFormat:@"%d",0];
    NSString* limit = @"30";
    [self fetchCollectionListInfo:limit offset:initialOffset];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else
    {
        ...
        if (will_cancel == YES)
        {
            ...
            [cell.selectButton addTarget:self action:@selector(confirmButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        }
        else
        {
...
}- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (![[[wineInfoArray objectAtIndex:indexPath.row]valueForKey:@"id" ]isEqualToString:@"0"])
    {
        [self performSegueWithIdentifier:@"collectionToSingle" sender:self];
    }
...
-(void)tableView:(UITableView*) tableView willDisplayCell:(UITableViewCell*)cell forRowAtIndexPath:(NSIndexPath*)indexPath
{
    if (indexPath.row == [wineInfoArray count] - 1)
    {
        ...
        [self fetchCollectionListInfo:limit offset:calculatedOffset];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"collectionToSingle"])
    {
        NSString * language = [[NSLocale preferredLanguages] objectAtIndex:0];
...
- (void)fetchCollectionListInfo:(NSString*)limit offset:(NSString*)offset
{
    if ([[defaults objectForKey:@"filtWithAllOptions"]isEqualToString:@"selected"])
    {
        [[VintageApiService sharedInstance]fetchCollectionListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:[defaults objectForKey:@"filtOrderBy"] order:[defaults objectForKey:@"filtOrder"] limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
    ...
    {
        [[VintageApiService sharedInstance]fetchCollectionListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] order_by:@"" order:@"" limit:limit offset:offset country_id:@"" wine_type_id:@"" ];
...
- (void)confirmButtonPressed:(UIButton*)sender
{
    ...
    for (int i=0 ; i < self.selectedButtonIdxArray.count; i++)
    {
        NSInteger selectedIndex = [self.selectedButtonIdxArray[i] integerValue];
        if (selectedIndex == sender.tag)
        {
            [self.selectedButtonIdxArray removeObjectAtIndex:i];
    ...
    {
        [self.selectedButtonIdxArray addObject:[NSString stringWithFormat:@"%ld",(long)sender.tag]];
        [sender setSelected:YES];
    }
    
    
    if (self.selectedButtonIdxArray.count == 0)
    {
        self.deleteButton.titleLabel.textColor = [UIColor whiteColor];
    }
    else
    {
        self.deleteButton.titleLabel.textColor = [UIColor colorWithHexString:@"a71645"];
    }
    
    NSLog(@"button tag:%ld",(long)sender.tag);
}
- (IBAction)filterAction:(id)sender {
    [self performSegueWithIdentifier:@"collectionToFilter" sender:self];
...
- (IBAction)cancelCollection:(UIButton *)sender {
    ...
    self.deleteButton.titleLabel.textColor = [UIColor whiteColor];
    self.bottomBarVIew.hidden = YES;
    [self.selectedButtonIdxArray removeAllObjects];
    [self.tableView reloadData];
...
- (IBAction)cancelButtonPressed:(UIButton *)sender {
    [sender setBackgroundColor:[UIColor colorWith8BitRed:0 green:0 blue:0 alpha:0.2]];
}

- (IBAction)cancelButtonTouchDragOutside:(UIButton *)sender {
    [sender setBackgroundColor:[UIColor clearColor]];
}

- (IBAction)cancelButtonTouchDragInside:(UIButton *)sender {
    [sender setBackgroundColor:[UIColor colorWith8BitRed:0 green:0 blue:0 alpha:0.2]];
}
...
- (IBAction)deleteCollection:(id)sender {
    ...
    NSString *selectedWineIndexString = @"";
    for (int i=0; i < self.selectedButtonIdxArray.count; i++)
    {
        if (i != 0)
        {
            selectedWineIndexString = [selectedWineIndexString stringByAppendingString:@","];
        }
        NSString *wineID = [[wineInfoArray objectAtIndex:[self.selectedButtonIdxArray[i] integerValue]] valueForKey:@"id"];
        selectedWineIndexString = [selectedWineIndexString stringByAppendingString:wineID];
        
    }
    [[NSUserDefaults standardUserDefaults]setObject:@"on" forKey:@"cancelSwitch"];
    [[NSUserDefaults standardUserDefaults]synchronize];
    [[VintageApiService sharedInstance]cancelCollectionListInfo:[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"] token:[[NSUserDefaults standardUserDefaults]objectForKey:@"access_token"] wine_ids:selectedWineIndexString];
    [self.selectedButtonIdxArray removeAllObjects];
}
...
- (IBAction)deleteButtonTouchDragOutside:(UIButton *)sender {
    [sender setBackgroundColor:[UIColor clearColor]];
}

- (IBAction)deleteButtonTouchDragInside:(UIButton *)sender {
    [sender setBackgroundColor:[UIColor colorWith8BitRed:0 green:0 blue:0 alpha:0.2]];
}
